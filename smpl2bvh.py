...
def smpl2bvh(model_path:str, poses, output:str, mirror:bool,
    ...
    poses.reshape(180,26,3).detach().numpy().astype(np.float32)
    ...
    # joints_mirror = np.array([(
    #     names.index("Left"+n[5:]) if n.startswith("Left") else (
    #     names.index("Right"+n[4:]) if n.startswith("Right") else 
    #     names.index(n))) for n in names])
    
    # mirror_rot = np.array([1, 1, -1, -1])
    # grot = quat.fk_rot(rots, parents)
    # grot_mirror = mirror_rot * grot[:,joints_mirror]
    # rots = quat.ik_rot(grot_mirror, parents)
